...
(function ($) {
	...
	$.MapMe.prototype = {
		init : function() {
			...
			// add poly gons to the map
			this.el.mapPolygons = this.setPolygons();
		...
		setPolygons : function() {
			var polygons = this.opts.polygons,
				polygonArray = [],
			...
			if (polygonLength > 0) {
				for (i = 0; i < polygonLength; i++) {
					...
					currentPolygon = new google.maps.Polygon(polygons[i].options);

					
					// add the polygon to the map
					currentPolygon.setMap(this.el.mapObject);

					// add events to the polygons
					if (polygons[i].events) {
						google.maps.event.addListener(currentPolygon, 'click', function(e) {
							map.setCenter(new google.maps.LatLng(-33.87116383262648, 151.22358798980713));
							map.setZoom(15);
						});
					}
					// add the current polygon to the array object
